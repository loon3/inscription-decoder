...
export function getInscription(raw){
    ...
    let witness = tx.ins[0].witness[1];

    //check if witness exists, if not check the next input
    if (!witness) {
        witness = tx.ins[1].witness[1];
    }
    ...
    //get file data
    script.pop()
    const headerData = script.lastIndexOf(0) + 1
    script.splice(0, headerData)
